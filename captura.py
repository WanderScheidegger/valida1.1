...
def identifica(model_out):
    ...
    percid = str(round(np.max(model_out)*100, 2)) + ' %'    
    return NOME.set(nomeid), SobreNome.set(snomeid), Perc.set(percid)
...
#    str_label = str(id[np.argmax(model_out)][2])
#        
#    print(str_label + " -- " + str(round(np.max(model_out)*100, 2))+ ' %.')
    print(str(model_out))
      

janela = tk.Tk()
janela.title("Aplicação")
janela.geometry("570x350+1+1")
...
tValida.grid(row=0, column=1,  pady=20)

botaoValidar = tk.Button(janela, text = "Validar", fg = "black", width=15, command = valida)
botaoValidar.grid(row=0, column=2, pady=20)
